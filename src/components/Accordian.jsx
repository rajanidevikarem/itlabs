...
const CustomAccordion = ({ title, date, location, jobcode,description, skills, isOpen, toggleAccordion }) => {
  return (
...
<div className='w-fit h-fit'> {item}</div>
          ...
          <ul>
            {skills.map((item, index) => (
              <li key={index} className=" py-4 "><strong>{item.label}: </strong>{item.value}</li>
            ))}
          </ul> 
